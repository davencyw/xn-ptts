...
fn main() -> Result<()> {
    ...
    let stream = match args.rng_values.as_ref() {
        None => tts.stream_with(&args.text, &opts)?,
        Some(path) => {
            let values: Vec<f32> = serde_json::from_str(&std::fs::read_to_string(path)?)?;
            let rng = ptts::rng::ReplayRng::new(values)?;
            tts.stream_with_rng(&args.text, &opts, Box::new(rng))?
        }
    };
    ...
    tracing::info!(
        "generated {duration:.2}s in {elapsed:.2}s (RTF={:.3}, first chunk {:.0}ms), peak RSS {:.0} MB",
        duration / elapsed,
