...
use crate::checkpoint::{self, Artifacts, ModelSource};
...
impl<Q: BackendQ> SynthOf<Q> {
    ...
    pub fn add_voice_file(&mut self, name: &str, path: &FsPath) -> Result<()> {
        if let Some(expected) = self.cfg.model_ext()
            && let Some(found) = checkpoint::voice_model_ext(path)?
            && found != expected
        {
            xn::bail!(
                "voice embedding {} was computed for model_ext '{found}' but this model is \
                 '{expected}'",
                path.display()
            );
        }
        let dev = self.model.device().clone();
        let emb = checkpoint::load_voice_embedding(path, &dev)?.to::<Q::T>()?;
    ...
    pub fn stream_with(&self, text: &str, opts: &SpeechOptions) -> Result<SpeechStream> {
        ...
        let rng = Box::new(GaussianRng::new(temperature, seed)?);
    ...
    /// fixed sequence (see [`crate::rng::ReplayRng`]) makes a generation
    ...
    pub fn stream_with_rng(
        ...
        let mimi_init = self.model.init_mimi_state(1, self.cfg.mimi.transformer_context)?;
        ...
        let decode_handle = std::thread::spawn(move || {
            ...
            while let Ok(frame) = latent_rx.recv() {
                match frame {
                    Frame::ChunkEnd => state = mimi_init.clone(),
                    Frame::Latent(latent) => {
...
///
/// Each item is one Mimi frame's worth of mono `f32` samples at
/// [`Self::sample_rate`]. The iterator ends when generation finishes; an `Err`
/// item is terminal.
...
impl SynthBuilder {
    ...
    fn load<Q: BackendQ>(mut self, device: Q::B) -> Result<SynthOf<Q>> {
        ...
        let vb = checkpoint::load_var_builder(&weights, device)?.root();
        ...
        vb.check_all_used_with_ignore(checkpoint::is_optional_tensor)?;
    ...
    fn take_tokenizer(
        ...
        if let Some(path) = path {
            return Ok(Box::new(crate::tok::Tok::from_file(path)?));
        }
        let _ = path;
        xn::bail!(
