...
pub fn decode_file(path: &std::path::Path) -> Result<(Vec<f32>, u32)> {
    ...
    // read failure and a clean finish look the same here.
...
///
/// Pair with [`crate::synth::Synth::voice_prompt_sample_rate`]:
///
/// ```no_run
/// # fn main() -> xn::Result<()> {
/// # let mut tts = ptts::synth::Synth::from_hub()?;
...
mod tests {
    ...
    fn resampling_scales_the_length() {
        // 2s of 16kHz silence to 24kHz should land near 2s again; the FFT
        // resampler works in blocks, so allow a block of slack.
        let pcm = vec![0f32; 32_000];
        let out = resample(&pcm, 16_000, 24_000).unwrap();
        let expected = 48_000f64;
        assert!(
            (out.len() as f64 - expected).abs() < 2048.0,
